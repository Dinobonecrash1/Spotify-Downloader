...
async def _(Mbot,message):
    ...
    if "shorts" in link:
        ...
        return await message.reply("Check out @Entertainment_AG(MOVIES)  @guardians_Bot_Updates(UPDATE) \n Please Support Us By joining these channel To Maintain This Project")
    try:
  ...
  #          await message.reply(id[2])
        ...
        #    await message.reply_audio(fileLink)
            AForCopy = await message.reply_audio(fileLink,caption=f"[{id[3]}](https://youtu.be/{id[0]}) - {id[2]} Thank you for using join our channel for movies and kdrama - @Entertainment_AG",title=id[3].replace("_"," "),performer=id[2],thumb=thumnail,duration=id[4])
            if DUMP_GROUP:
        ...
        if os.path.exists(randomdir):
           rmtree(randomdir)
        await message.reply("Check out @Entertainment_AG(MOVIES)  @guardians_Bot_Updates(UPDATE) \n Please Support Us By joining these channel To Maintain This Project")
    except Exception as e:
        print(e)
        if LOG_GROUP:
