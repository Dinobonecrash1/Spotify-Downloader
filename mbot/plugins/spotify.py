  ...
  #     T = await message.reply_text(f"[{song.get('name')} - {song.get('artist')}](https://open.spotify.com/track/{song.get('deezer_id')}) Track Not Found ⚠️")
    ...
    #    await message.reply_text(f"try: `/saavn {song.get('name')}`")
        ...
        try:
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Feedback", callback_data="feed")]]))
            await message.reply_text(f"Check out @Entertainment_AG(MOVIES)  @guardians_Bot_Updates(UPDATE)")
            await m.delete()
        except:
            pass 
...
async def feedback(Mbot,query):
      try:
          K = await query.message.edit(f"Feedback 🏴‍☠️",
                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Press here", url="https://t.me/Zenitsu_AF")]]))
...
async def bug(_,query):
      try:                                                                                                                                  
          ...
          H = await query.message.edit(f"Bug Report 🪲",
                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Report to dev ", url="https://t.me/Zenitsu_AF")]]))
          if BUG:
             await copy(K,H)
      except Exception as e:
          pass
